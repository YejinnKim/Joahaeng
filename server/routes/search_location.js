    ...
    db.query('select region_ID from travel_region where region_name = ?', region, (err, result) => {  
        ...
        request(
            {url: url, method: 'GET'}, (error, response, body) => {
                var dataList = JSON.parse(body).response.body.items.item
                if (!location)
                    res.render('search_location', {data: dataList, region: region, location: 0})
                else {
                    res.render('search_location', {data: dataList, region: region, location: location})
                }
            }
        )
    })
